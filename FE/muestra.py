...
def ventana_catalogo(root):
    ...
    def cargar_catalogo():
        try:
            ...
            for libro in libros:
                # por si algún campo tiene otro nombre, se muestra vacío pero no falla
                tabla.insert(
                    "",
                    "end",
                    values=(
...
def ventana_actualizar_ejemplares(root):
    ...
    def guardar_cambios():
        try:
            ...
            # El autor que se envía sigue siendo autor_id, así que si solo tienes el id lo dejas
            # Si quieres mapear nombre → id necesitarías otra tabla/endpoint
            # Aquí mantenemos el autor_id original:
            # libro["autor_id"] = libro.get("autor_id", 1)
...
def ventana_reportes(root):
    ...
    def cargar_reportes():
        try:
            ...
            sin_stock = sum(1 for l in libros if l.get("stock_disponible", 0) == 0)
